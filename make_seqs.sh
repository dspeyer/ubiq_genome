...
for i in *.bed.1; do bedtools getfasta -fi ~awesome/hg19.fa -bed $i -fo `echo $i | sed 's/bed.1/ref.fa/'` -s; done
head -n 1 *.ref.fa | grep '(+)' -B 1 | grep = | awk '{print $2}' | sed 's/.ref.fa//' > poshitlist
